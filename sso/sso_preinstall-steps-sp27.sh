...
. ${CONFIG_FILE}
#If these are changed in COnfig file will have problems
if [ -z ${httpd_unit_0_ip} ]; then 
echo "problem with variable httpd_unit_0_ip in ${CONFIG_FILE}"
exit 1
fi
if [ -z ${sso_unit_0_ip} ]; then
echo "problem with variable sso_unit_0_ip in ${CONFIG_FILE}"
exit 1
fi
if [ -z ${sso_unit_1_ip} ]; then
echo "problem with variable sso_unit_1_ip in ${CONFIG_FILE}"
exit 1
fi
...
do
   ...
   ssh ${node} "mkdir -p ${UI_PROPERTY_LOCATION_ON_PEER_NODES}"
   scp ${UI_PROPERTY_FILE} ${node}:${UI_PROPERTY_LOCATION_ON_PEER_NODES}
...
PEER1_COMMAND="ssh ${PEER1} 'echo -e \"${httpd_unit_0_ip}      ${APACHE_FQDN}\n${sso_unit_0_ip}    ${JBOSS_SSO_ALIAS}\n${LDAP_IP}   ${LDAP_HOSTNAME}\n${LDAP_IP}   ldap1\n${LDAP_IP}   ldap2\n${LDAP_IP}   ${LDAP_HOSTNAME}\" >> /etc/hosts'"
PEER2_COMMAND="ssh ${PEER2} 'echo -e \"${httpd_unit_0_ip}      ${APACHE_FQDN}\n${sso_unit_1_ip}    ${JBOSS_SSO_ALIAS}\n${LDAP_IP}   ${LDAP_HOSTNAME}\n${LDAP_IP}   ldap1\n${LDAP_IP}   ldap2\n${LDAP_IP}   ${LDAP_HOSTNAME}\" >> /etc/hosts'"

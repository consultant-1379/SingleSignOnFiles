...
echo "Ping succesful with ${SERVER}"
}
...
LIST_OF_SECURITY_FILES="rootca.cer"
...
print "Checking can ping ${UI_PRES_SERVER}"
check_ping ${UI_PRES_SERVER}

print "Checking can ping sso.${UI_PRES_SERVER}"
check_ping sso.${UI_PRES_SERVER}
...
curl -k "http://${UI_PRES_SERVER}" -I -s | grep Location > /tmp/ssopostinstall_check

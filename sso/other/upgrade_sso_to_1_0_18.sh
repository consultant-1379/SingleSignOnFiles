...
if [ `ls ${UPGRADE_SOURCE} | grep -c ERICps_` -ne 0 ]; then

  NEW_PS_VERSION=`ls ${UPGRADE_SOURCE} | grep ERICps_ | cut -d- -f2 | sed s/.rpm//g`
  ${ECHO} -e "Pre-upgrade versions of Presentation Server\n"
  litp /definition/pres_serv/pkg/ show
  litp /definition/pres_serv/de/ show
  ${ECHO} -e "Upgrading Single Sign to ${NEW_PS_VERSION} \n"
  litp /definition/pres_serv/pkg/ update version="${NEW_PS_VERSION}"
  litp /definition/pres_serv/de/ update version="${NEW_PS_VERSION}" install-source="/opt/ericsson/com.ericsson.nms.pres.PresentationServer/presentation-server-ear-${NEW_PS_VERSION}.ear" name="presentation-server-ear-${NEW_PS_VERSION}.ear"
fi
...
${ECHO} -e "Updating properties files to 1.0.18"
/opt/ericsson/nms/litp/bin/deployment/sso/update_properties_to_1_0_18.sh
...
${ECHO} -e "Post-upgrade versions of Presentation Server\n"
litp /definition/pres_serv/pkg/ show
litp /definition/pres_serv/de/ show
